...
export async function getBucket(id: string) {
  ...
  if (data && !Number.isNaN(Number.parseFloat(data.tokens))) {
    tokens = Number.parseFloat(data.tokens);
    lastUpdated = Number.parseInt(data.lastUpdated);
  }
  ...
  await updateBucket(id, tokens, lastUpdated);
